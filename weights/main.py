...
def train(epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        r_neigbor = []
        r_neigbor.append(target_left)
        r_neigbor.append(target_right)        
        if cuda:
        ...
        dflow = []
        dflow.append(disc_flow[2])
        dflow.append(disc_flow[3])

        discriminator_loss = adversarial_criterion(discriminator(high_res_real,r_neigbor,dflow), target_real) + \
                             adversarial_criterion(discriminator(Variable(high_res_fake.data),r_neigbor,dflow), target_fake)
        generator_adv_loss = 1e-3*adversarial_criterion(discriminator(Variable(high_res_fake.data),g_neigbor,dflow), ones_const)
        vgg_loss = 0.006*content_criterion(fake_features, real_features)

        loss = criterion(prediction, target) + vgg_loss + generator_adv_loss
        t1 = time.time()
        ...
        print("===> Epoch[{}]({}/{}): Loss: {:.4f}, VGG Loss {:.8f}, genAdv Loss {:.8f}, discAdv Loss {:.8f}  || Timer: {:.4f} sec.".format(epoch, iteration, len(training_data_loader), loss.data,vgg_loss, generator_adv_loss,discriminator_loss,  (t1 - t0)))
